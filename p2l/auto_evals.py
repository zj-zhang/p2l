...
from p2l.auto_eval_utils import registered_simple_metrics, registered_aggr_metrics, registered_helpers
...
def parse_model_list(hf_model, local_path):
    ...
    if not model_list_path:
        model_list_path = hf_hub_download(repo_id=hf_model, filename="model_list.json", repo_type="model")
...
def parse_eval_output_data(model_repo, local_eval_path, local_checkpoint_path, hf_checkpoint_repo, 
                           hf_checkpoint_file, loss_type, model_list):
    ...
    if local_checkpoint_path or hf_checkpoint_repo:
        ...
        if not path:
            if not hf_checkpoint_file:
                raise ValueError("Must provide checkpoint file along with checkpoint repo")
            path = hf_hub_download(repo_id=hf_checkpoint_repo, filename=hf_checkpoint_file, repo_type="dataset")
        ...
        if "eval_output.jsonl" not in files:
            raise FileNotFoundError(f"'eval_output.jsonl' not found in the hf repository'{model_repo}'.")
        path = hf_hub_download(repo_id=model_repo, filename="eval_output.jsonl", repo_type="model")
...
def parse_train_data(hf_data, local_path, loss_type, model_list):
    if not hf_data and not local_path:
        warnings.warn("No train data provided, marginal model type will not work if specified") 
        return 
    ...
    return add_labels_to_data(data, loss_type, model_list)
...
def parse_arena_data(path, initial_rating=1000, BASE=10, SCALE=400):
    ...
    pivot = df.pivot(index="model_name", columns="category", values="beta").reindex(model_list)
    ...
    category_to_betas = {category: torch.tensor(pivot[category].values, dtype=torch.float)
...
def filter_battle_data(battles, category):
    ...
    try:
        ...
        if key == "math":
            return battles[battles['category_tag'].apply(lambda x: x['math_v0.1']['math'])]
        if key == "complexity":
            return battles[battles['category_tag'].apply(lambda x: x['criteria_v0.1']['complexity'])]
        if key == "creative_writing":
            return battles[battles['category_tag'].apply(lambda x: x["creative_writing_v0.1"]["creative_writing"])]
        if key == "hard":
            return battles[
...
def simple_metrics(metrics, output, labels, loss_type):
    ...
    for metric in tqdm(metrics, desc="Simple Metrics", unit="metrics"):
        ...
        results[metric] = round(metric_val, 4) if isinstance(
            metric_val, float) else metric_val

    return results

def category_metrics(metrics, output, labels, loss_type, model_type, model_list):
    results = {}
    ...
    model_output = aggr_func_model(head_output=output, labels=labels, model_list=model_list, loss_type=loss_type)
    gt_output = aggr_func_gt(labels=labels, model_list=model_list, loss_type=loss_type)

    for metric in tqdm(metrics, desc="Category Metrics", unit="metric"):
        metric_dict = registered_aggr_metrics[loss_type]
        metric_func = metric_dict[metric]
        metric_val = metric_func(gt_output=gt_output, model_output=model_output, model_list=model_list,
                                 loss_type=loss_type, labels=labels)
        results[metric] = round(metric_val, 4) if isinstance(
            metric_val, float) else metric_val
        
    return results

def random_subset_metrics(metrics, output, labels, subset_sizes, trials_per_subset, loss_type, model_type, model_list):
    results = {}
    ...
    for idx, size in enumerate(subset_sizes):
        ...
        for _ in tqdm(range(trials_per_subset[idx]), desc=f"Random Subset size {size}", unit="trial"):
            sample_output, sample_labels = get_subset_prompts(output, labels, size)
            
            model_output = aggr_func_model(head_output=sample_output, labels=sample_labels, model_list=model_list, loss_type=loss_type)
            gt_output = aggr_func_gt(labels=sample_labels, model_list=model_list, loss_type=loss_type)

            for metric in metrics:
                metric_dict = registered_aggr_metrics[loss_type]
                metric_func = metric_dict[metric]
                metric_val = metric_func(gt_output=gt_output,
                                         model_output=model_output, model_list=model_list, loss_type=loss_type)

                
                subset_results[metric] += metric_val
               
        for metric in metrics:
            subset_results[metric] = round(subset_results[metric] / trials_per_subset, 4)

        results[size] = subset_results

    return results

def aggr_scale_metrics(metrics, output, labels, subset_sizes, trials_per_subset, loss_type, model_type, model_list, arena_rankings, gt):
    results = {}
    ...
    # TODO: cleanup
    if gt == "arena":
        gt_output = HeadOutputs(coefs=arena_rankings, eta=None)
    else:
        aggr_func_gt = registered_helpers["marginal-gt"]['aggregrate']
        gt_output = aggr_func_gt(labels=labels, model_list=model_list, loss_type=loss_type)
    
    # TODO: arbitray threshold to limit memory consumption for batching
    max_prompts_times_samples_squared = 250000
    ...
    for idx, size in enumerate(subset_sizes):
        ...
        for metric in metrics:
            subset_results[metric] = round(subset_results[metric] / float(trials_per_subset[idx]), 4)

        results[size] = subset_results

    return results

def get_metrics(val_data, train_data, arena_rankings, model_list, args):
    results = {}
    
    to_inc = set(args.metrics_to_inc)
    output_label_func = registered_helpers[args.model_type]['output_labels']
    output, labels = output_label_func(val_data=val_data, train_data=train_data, arena_rankings=arena_rankings,
                                              loss_type=args.loss_type, model_list=model_list)
    
    if "simple" in to_inc:
        simple_results = simple_metrics(metrics=args.simple_metrics, output=output, 
                                        labels=labels, loss_type=args.loss_type)
        results['simple_metrics'] = simple_results
    
    if "category" in to_inc:
        category_results = category_metrics(metrics=args.category_metrics, loss_type=args.loss_type, 
                                            model_type=args.model_type, model_list=model_list, output=output, labels=labels)
        results['category_metrics'] = category_results
        
    if "random_subsets" in to_inc:
        subset_results = random_subset_metrics(metrics=args.rand_subset_metrics, subset_sizes=args.rand_subset_sizes, 
                                        trials_per_subset=args.rand_num_samples, loss_type=args.loss_type,
                                        model_type=args.model_type, model_list=model_list, output=output, labels=labels)
        results['random_subsets'] = subset_results
    
    if "aggr_scale" in to_inc:
        scale_results = aggr_scale_metrics(metrics=args.aggr_scale_metrics, subset_sizes=args.aggr_scale_subset_sizes, 
                                        trials_per_subset=args.aggr_scale_num_samples, loss_type=args.loss_type,
                                        model_type=args.model_type, model_list=model_list, output=output, labels=labels, 
                                        arena_rankings=arena_rankings, gt=args.aggr_scale_gt)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_file_name", type=str, nargs='+', default=["eval_metrics"])
    ...
    parser.add_argument("--model_type", type=str, default="p2l", help="p2l, marginal, arena")
    
    parser.add_argument("--categories", nargs='*',
                        default=['all', 'creative_writing', 'math', 'language=Chinese', 'is_code', 'hard'])

    parser.add_argument("--simple_metrics", nargs='*', default=['Loss', 'BCELoss','MSELoss', 'Accuracy', 'Mean-BT', 'Std-BT', 'Spread-BT',
                                                                'Mean-Spread-BT', 'Mean-IQR-BT', 'Mean-Std-BT'])
    # gt is marginal on val
    parser.add_argument("--category_metrics", nargs='*', default=['Leaderboard', "Aggr_Loss", "Aggr_BCELoss", 'L1-Dist-Prob', 'Spearman-lbs', 'Kendall-lbs', 'IQR-BT',
                                                                  'Std-BT', 'Spread-BT', 'Top-k-fraction', 'Top-k-displace'])
                                
    parser.add_argument("--rand_subset_sizes", nargs='*',
                        default=[250, 500, 1000, 2000])
    
    parser.add_argument("--rand_num_samples", nargs='*',
                        default=[50, 20, 5, 3])
    parser.add_argument("--rand_subset_metrics", nargs='*',
                        default=['L1-Dist-Prob', 'Spearman-lbs', 'Kendall-lbs'])
    # gt is arena leaderboard
    parser.add_argument("--aggr_scale_subset_sizes", nargs='*',
                        default=[1, 10, 25, 100, 250, 500, 1000, 2000])
    parser.add_argument("--aggr_scale_num_samples", nargs='*',
                        default=[500, 500, 500, 200, 100, 40, 10, 6])
    
    parser.add_argument("--aggr_scale_metrics", nargs='*',
                        default=['L1-Dist-Prob', 'Spearman-lbs', 'Kendall-lbs'])
    parser.add_argument("--aggr_scale_gt", type=str, default="marginal-gt")
    
    parser.add_argument("--metrics_to_inc", nargs='*', default=['simple', 'category', 'random_subsets', 'aggr_scale'])
    ...
    for idx in range(len(args.output_file_name)):
        ...
        model_list = parse_model_list(args.model_repo, args.model_list_path)
        ...
        hf_checkpoint_file = args.hf_checkpoint_file[idx] if args.hf_checkpoint_file else None
        ...
        val_data, model_list = parse_eval_output_data(args.model_repo, eval_path, checkpoint_path, 
                                                args.hf_checkpoint_repo, hf_checkpoint_file, args.loss_type, model_list)
        
        train_data = parse_train_data(args.hf_train_dataset, args.train_path, args.loss_type, model_list)
        ...
        for category in args.categories:
            ...
            models[current_model] = get_metrics(cat_val_data, cat_train_data, arena_rankings, model_list, args)   
        
        results['models'] = models   
        save_output(results, args.output_dir, args.hf_output_dir, args.output_file_name[idx])
