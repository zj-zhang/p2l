...
from p2l.model import registered_losses, HeadOutputs, registered_aggr_models, registered_pairwise_losses
...
def output_labels_p2l(val_data: pd.DataFrame,
                         **kwargs):
...
def output_labels_marginal(val_data: pd.DataFrame,
                          train_data: pd.DataFrame,
                          model_list: np.array,
                          loss_type: str,
                          **kwargs):
   train_labels = torch.tensor(np.stack(train_data["labels"]))
   coefs, eta = train_marginal(model_list, train_labels, loss_type)
...
def output_labels_marginal_gt(val_data: pd.DataFrame,
                          model_list: np.array,
                          loss_type: str,
                          **kwargs):
   val_labels = torch.tensor(np.stack(val_data["labels"]))
...
def output_labels_arena(arena_rankings: torch.tensor,
   ...
   etas = None # arean ranking don't have eta
...
def loss(head_output: HeadOutputs,
           labels: torch.Tensor,
           loss_type: str,
           **kwargs):
...
def BCE_loss(head_output: HeadOutputs,
           labels: torch.Tensor,
           **kwargs):
   non_tie_index = torch.where(labels[:, 2] == 0)[0]
...
def Aggr_Loss(gt_output: HeadOutputs,
               model_output: HeadOutputs,
               loss_type: str,
               labels: torch.tensor,
               **kwargs):
   
   loss_func = registered_simple_metrics[loss_type]["Loss"]
   
   gt_loss = loss_func(labels=labels, head_output=expand_output(gt_output, labels), loss_type=loss_type)
   model_loss = loss_func(labels=labels, head_output=expand_output(model_output, labels), loss_type=loss_type)
   
   return {"ground-truth": round(gt_loss, 4), "model-aggr": round(model_loss, 4)}
...
def Aggr_BCE_Loss(gt_output: HeadOutputs,
               model_output: HeadOutputs,
               loss_type: str,
               labels: torch.tensor,
               **kwargs):
   
   loss_func = registered_simple_metrics[loss_type]["BCELoss"]
...
def RK_accuracy(
   ...
   **kwargs
   ...
   pred_labels = torch.where(
       p_w >= p_l,
       torch.where(p_w >= p_t, 1, 0.5),
       torch.where(p_l >= p_t, 0, 0.5)
   )
...
def beta_mean_spread(
   ...
   **kwargs,
   ...
   betas = head_output.coefs
   max_min_per_prompt = torch.max(
       betas, dim=-1).values - torch.min(betas, dim=-1).values
...
def beta_mean_iqr(
   ...
   **kwargs,
   ...
   betas = head_output.coefs
   iqr_per_prompt = torch.quantile(
       betas, 0.75, dim=-1) - torch.quantile(betas, 0.25, dim=-1)
...
def aggr_marginal_gt(labels: torch.Tensor,
...
def aggr_p2l(head_output: HeadOutputs,
               labels: torch.Tensor,
               model_list: torch.Tensor,
               loss_type: str,
               **kwargs):
   coefs, eta = train_aggr_prob(model_list, head_output, labels, loss_type)
   return HeadOutputs(coefs=coefs[0], eta=eta[0] if eta is not None else None)

@register_helper("p2l", "aggregrate-batch")
def aggr_p2l_batch(head_output: HeadOutputs,
               labels: torch.Tensor,
               model_list: torch.Tensor,
               loss_type: str,
               **kwargs):
   coefs_batch, eta_batch = train_aggr_prob_batch(model_list, head_output, labels, loss_type)
   return [HeadOutputs(coefs=coefs_batch[i], eta=eta_batch[i] if eta_batch is not None else None) for i in range(coefs_batch.shape[0])]

@register_helper("arena", "aggregrate")
@register_helper("marginal", "aggregrate")
def aggr_non_p2l(head_output: HeadOutputs,
               **kwargs):
   # arena just has the same betas repeated
   return HeadOutputs(coefs=head_output.coefs[0, :], eta=None)


def train_marginal(model_list, labels, loss_type, lr=1.0, tol=1e-8, max_epochs=50):
   model_cls = registered_aggr_models[loss_type]
   model = model_cls(len(model_list))

   optimizer = optim.LBFGS(model.parameters(), lr=lr, max_iter=max_epochs, tolerance_grad=tol, tolerance_change=tol)
   prev_loss = float("inf")

   loss_func = registered_losses[loss_type]
   labels = labels.squeeze() # marginal doesn't use batching since singular at onc
...
def train_aggr_prob(model_list, head_outputs, labels, loss_type, lr=1.0, tol=1e-8, max_epochs=50):
   model_cls = registered_aggr_models[loss_type]
   model = model_cls(len(model_list))
   ...
   # add a batch size of 1 since aggregration is done in batches
   true_probs = true_probs_func(real_output = head_outputs).unsqueeze(0)
   
   def closure():
       optimizer.zero_grad()
       coefs, eta = model()
       aggr_output = HeadOutputs(coefs=coefs, eta=eta)
       loss = loss_func(real_output=head_outputs, aggregated_output=aggr_output, true_probs=true_probs)
       loss.backward()
       return loss
   ...
   true_coefs, true_eta = model()
   return true_coefs.detach(), true_eta.detach() if true_eta else None


def train_aggr_prob_batch(model_list, head_outputs, labels, loss_type, lr=1.0, tol=1e-8, max_epochs=50):
   batch_size = head_outputs.coefs.shape[0]
   model_cls = registered_aggr_models[loss_type]
   ...
   true_probs = pair_probs_func(real_output = head_outputs)
...
def rk_eta(output):
   if output.eta is None:
      return None
   BETA = 0.1
   return torch.clamp(torch.nn.functional.softplus(output.eta - 22.5, BETA).squeeze(-1), min=0.02)
...
def leaderboard(gt_output: HeadOutputs,
               model_output: HeadOutputs,
               model_list: np.array,
               **kwargs):
...
def get_leaderboard(output, model_list):
   ...
   for i in range(len(sorted_model_names)):
       beta = round(sorted_betas[i].item(), 4) if not torch.isnan(sorted_betas[i]) else "nan"
       cur_model = str(sorted_model_names[i]) + \
           ": " + str(beta)
...
def l1_dist_prob_bt(gt_output: HeadOutputs,
                model_output: HeadOutputs,
                **kwargs):
...
def l1_dist_prob_rk(gt_output: HeadOutputs,
                model_output: HeadOutputs,
                loss_type: str,
                **kwargs):
   eta1 = gt_output.eta
...
def l1_dist_prob_bag(gt_output: HeadOutputs,
                model_output: HeadOutputs,
                loss_type: str,
                **kwargs):
   eta1 = gt_output.eta
...
def beta_iqr(gt_output: HeadOutputs,
            model_output: HeadOutputs,
            **kwargs):
   gt_coefs, model_coefs, = gt_output.coefs, model_output.coefs
   gt_iqr = (torch.quantile(gt_coefs, 0.75) -
                   torch.quantile(gt_coefs, 0.25)).item()
   model_iqr = (torch.quantile(model_coefs, 0.75) -
              torch.quantile(model_coefs, 0.25)).item()
...
def beta_std_aggr(gt_output: HeadOutputs,
                model_output: HeadOutputs,
                **kwargs):
  
   gt_betas, model_betas = gt_output.coefs, model_output.coefs
   gt_std, model_std = torch.std(gt_betas.flatten()).item(), torch.std(model_betas.flatten()).item()
...
def beta_spread_aggr(gt_output: HeadOutputs,
                model_output: HeadOutputs,
                **kwargs):
   gt_betas, model_betas = gt_output.coefs.flatten(), model_output.coefs.flatten()
  
   gt_spread, model_spread = torch.max(gt_betas) - torch.min(gt_betas), torch.max(model_betas) - torch.min(model_betas)
   return {"ground-truth": round(gt_spread.item(), 4), "model-aggr": round(model_spread.item(), 4)}
...
def kendall_lb(gt_output: HeadOutputs,
              model_output: HeadOutputs,
              **kwargs):
...
def spearman_lb(gt_output: HeadOutputs,
               model_output: HeadOutputs,
               **kwargs):
   gt_betas, model_betas = remove_beta_nan(gt_output.coefs, model_output.coefs)
   gt_lb = gt_betas.numpy()
...
def top_k_frac(gt_betas: torch.tensor,
...
def top_k_displace(gt_betas: torch.tensor,
               model_betas: torch.tensor,
               k:int):
...
def top_k_frac_dict(gt_output: HeadOutputs,
               model_output: HeadOutputs,
               **kwargs):
   gt_betas, model_betas = remove_beta_nan(gt_output.coefs, model_output.coefs)
...
def top_k_dist_dict(gt_output: HeadOutputs,
               model_output: HeadOutputs,
               **kwargs):
   gt_betas, model_betas = remove_beta_nan(gt_output.coefs, model_output.coefs)
